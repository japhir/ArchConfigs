...
Let's see how the symbols look in my regular font:
NEXT: ☐
TICK: →
WAIT: ☕
SOME: ⋯
   ...
   (setq org-capture-templates
         '(;("a" "Appointment" entry (file org-in-file)
           ...
           ("w" "Website" entry (file org-in-file)
            "* %?\nEntered on %U\n %i\n %a")
           ("r" "Daily Review" entry (file "~/org/log.org")
            (file "~/org/daily_review.org") :clock-in t :clock-keep t)
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    (setq mu4e-change-filenames-when-moving t ; important for isync
          ...
          mu4e-compose-format-flowed nil  ; plain-text nice to read on phone
