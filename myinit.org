...
* package archive
** compat
Emacs 30.x ships compat 30.x built-in. Many packages (orderless, magit, etc.)
require compat >= 31. Install from source to shadow the built-in.
#+begin_src emacs-lisp
  (elpaca (compat :type git :host github :repo "emacs-compat/compat" :inherit nil))
  (elpaca-wait)
#+end_src
...
#+begin_src emacs-lisp
  (use-package vterm
    :custom
    ;; build vterm-module.so non-interactively; otherwise a batch byte-compile
    ;; that requires vterm hangs forever on the "build it? (y/n)" prompt
...
#+begin_src emacs-lisp
  ...
  ;; let tiling WMs (AeroSpace, niri) fill column exactly instead of
  ;; snapping frame size to a multiple of char height
  (setq frame-resize-pixelwise t
        window-resize-pixelwise t)
  (scroll-bar-mode -1)   ; turn off the scroll bar
  (tool-bar-mode -1)     ; turn off the tool bar
  (if (not (eq system-type 'darwin))	; on macos we do want a menu-bar
...
#+begin_src emacs-lisp
  (use-package circadian
    :config
    (require 'solar)
    (setq calendar-latitude 52.08             ;; Utrecht
...
#+begin_src emacs-lisp
  (use-package visual-fill-column
    :ensure (:type git :host github :repo "joostkremers/visual-fill-column" :inherit nil)
...
#+begin_src emacs-lisp
  (use-package transient
    :ensure (:type git :host github :repo "magit/transient" :inherit nil))
  (use-package magit
...
#+begin_src emacs-lisp
  (defun internet-up-p (&optional host)
    (= 0 (call-process "ping" nil nil nil "-c" "1"
                       ;; -W is seconds on Linux, milliseconds on macOS
                       (if (eq system-type 'darwin) "-W" "-W") "1"
...
#+end_src
*** agenda files
all the org-files in my org-directory. Rebuilt dynamically before every
agenda build (see "dynamic agenda files" under :config), so files created
after init — e.g. new dailies — are always included.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (org-agenda-custom-commands
        '(("i" "Inbox" tags "inbox")
          ...
          ("gs" "Software" tags-todo "software")
          ...
          ("pr" "Rens" tags-todo "Rens")
          ("pk" "Renske" tags-todo "Renske")
          ("pj" "Jonathan" tags-todo "Jonathan")
          ("pf" "Frans" tags-todo "Frans")))
#+end_src
*** stuck projects
#+begin_src emacs-lisp
 ...
 #+begin_src emacs-lisp
   (org-capture-templates
    '(;("a" "Appointment" entry (file org-in-file)
      ...
      ("d" "Daily Review" entry (file "~/org/log.org")
       (file "~/org/daily_review.org") :clock-in t :clock-keep t :jump-to-captured t)
...
#+begin_src emacs-lisp
  ...
  (org-tag-alist '(("prj" . ?j)
                   ;; WBSO / export tags (see ~/org/wbso-export.org and the
                   ;; Note Workflow v2 spec, D6/D9/D23/D24)
                   ("software" . ?s)   ;; claimable R&D work (Emdesk :R:)
                   ("research" . ?g)   ;; claimable R&D work (Emdesk :R:)
                   ("meeting" . ?M)    ;; context-only in export, no effort claim
                   ("timeregistration" . ?t)
                   ("fail" . ?x)       ;; failed experiment — WBSO gold
                   ("private" . ?v)    ;; never exported
                   (:startgroup . nil)
                   ("@home" . ?h)
                   ("@office" . ?o)
                   ("@errands" . ?e)
...
Rescan ~/org before every agenda build so newly created files (dailies!)
show up without restarting or re-running init. Lockfiles (.#foo.org) are
dangling symlinks that would break the agenda, so they're filtered out.
#+begin_src emacs-lisp
  (defun my/org-agenda-files-refresh (&rest _)
    "Rebuild variable `org-agenda-files' from the contents of ~/org."
  ...
  (advice-add 'org-todo-list :before #'my/org-agenda-files-refresh)
#+end_src
*** effort estimates
#+begin_src emacs-lisp
...
#+end_src
*** ox-pandoc
docx via pandoc; the org2gdoc skill's filter drops heading anchors that Drive imports as bookmark icons
#+begin_src emacs-lisp
  (use-package ox-pandoc
    :after ox
    :config
    ;; absolute path: pandoc gets the argument without shell expansion, so ~ fails silently
    (setq org-pandoc-options-for-docx
          `((lua-filter . ,(expand-file-name "~/.agents/skills/org2gdoc/strip-anchors.lua")))))
#+end_src
*** ox-extra
...
** vulpea (work laptop only)
:PROPERTIES:
:CREATED:  [2026-07-09 Thu]
:END:
[[https://github.com/d12frosted/vulpea][vulpea]] v2 (2.5.0+, the July-2026 standalone rewrite): own SQLite DB,
async indexing, runs *side by side* with org-roam over the same files
(D15). Used by [[file:~/org/wbso-export.org][wbso-export.org]] to resolve project-node titles → IDs and
to find unlinked plaintext mentions. org-roam above is untouched.

Machine-scoped to this work mac (=system-type 'darwin=) per D21; the
personal Arch machine keeps its inbox/todo/work GTD for now. To migrate
later, lift the =when= guard (see the spec's D21 migration steps).

Notes:
- =vulpea-buffer-alias-property= points at =ROAM_ALIASES= so vulpea reads
  the same aliases org-roam wrote (D14; load-bearing for unlinked-mention
  detection of old title spellings).
- vulpea keeps its DB in =user-emacs-directory= — *separate* from
  =~/org-roam.db=, so the two databases never collide.
- =rg= (ripgrep) is required for unlinked mentions; =fswatch= is optional
  (external-change detection). Without =fswatch= vulpea polls, which is
  fine here.
#+begin_src emacs-lisp
  (when (eq system-type 'darwin)
    ;; Point vulpea at the notes dir and the org-roam alias property BEFORE it
    ;; loads, so its defaults compute correctly.
    (setq vulpea-db-sync-directories (list (file-truename "~/org/notes/")))
    (setq vulpea-buffer-alias-property "ROAM_ALIASES") ; D14
    (use-package vulpea
      :ensure (vulpea :host github :repo "d12frosted/vulpea"
                      :ref "v2.5.0"
                      :files (:defaults))
      :defer t
      :commands (vulpea-find vulpea-insert vulpea-db-sync-full-scan)
      :init
      (setq vulpea-db-location
            (expand-file-name "vulpea/vulpea.db" user-emacs-directory))
      :config
      ;; background reindex + external-change detection (D15). On first run
      ;; this scans ~/org/notes/ in the background to build the DB.
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    (global-corfu-mode)
    :config
    ;; corfu-terminal is deliberately not installed (:ensure nil), and a daemon's
    ;; initial frame is non-graphic, so require it defensively rather than failing init.
    ...
    (customize-set-variable 'corfu-auto-prefix 4)  ; Complete with less prefix keys
    ...
    (defvar my-corfu-reclaim-focus nil)
    (add-hook 'completion-in-region-mode-hook
      (lambda ()
        (unless completion-in-region-mode
          (setq my-corfu-reclaim-focus t))))
    (add-function :after after-focus-change-function
      (lambda ()
        (when my-corfu-reclaim-focus
          (setq my-corfu-reclaim-focus nil)
...
#+begin_src emacs-lisp
  (use-package ess
    ...
    :init
    ;; upstream moved ess-jags-d.el to lisp/obsolete/, which elpaca links into a
    ;; subdirectory that is not on load-path -- but ess-site.el still requires it.
    (when-let* ((lib (locate-library "ess-mode"))
                (dir (expand-file-name "lisp/obsolete" (file-name-directory lib)))
...
:END:
:LOGBOOK:
- Note taken on [2026-03-03 Tue 21:49] \\
  finally updated this so it works with builtin setup
:END:
