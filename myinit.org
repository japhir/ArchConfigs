...
#+END_SRC
*** COMMENT convert utf-8 to latex entries
This works, but is obviated by my new custom entities.
#+BEGIN_SRC emacs-lisp
  (defun my-latex-filter-nobreaks (text backend info)
    "Ensure \" \" are properly handled in LaTeX export."
    (when (org-export-derived-backend-p backend 'latex)
      (replace-regexp-in-string " " "~" text)))
  (defun my-latex-filter-delta (text backend info)
    "Ensure \"δ\" are converted to \"\\delta\" in LaTeX export."
    (when (org-export-derived-backend-p backend 'latex)
      (replace-regexp-in-string "δ" "\\delta" text t t)))
  (defun my-latex-filter-Delta (text backend info)
    "Ensure \"Δ\" are converted to \"\\Delta\" in LaTeX export."
    (when (org-export-derived-backend-p backend 'latex)
      (replace-regexp-in-string "Δ" "\\Delta" text t t)))
